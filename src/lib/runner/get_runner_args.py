...
def get_train_args_WSB(args, optimizer_G, optimizer_D, scheduler_G, scheduler_D, save_dir, device):
    ...
    if str(args.model) == 'i2iwsb':
        # I2IWSB
        trainer_args['lamb'] = str(args.lamb)  # gradient penalty
    ...
    if hasattr(args, 'reuse') and eval(args.reuse):  # additional training
...
def get_test_args_WSB(args, save_dir, device, test_dataset):
    ...
    if str(args.model) == 'i2iwsb':
        # I2IWSB
        tester_args['lamb'] = str(args.lamb)  # gradient penalty
